...
import { Cookie, Settings, Eye, Shield, Database, Phone } from "lucide-react";
...
export const CookiePolicy = () => {
  ...
  const sections = [
    ...
    },
    {
      ...
      content: t('cookiePolicy.sections.typesOfCookies.content')
    },
    {
      icon: Eye,
    ...
    {
      icon: Settings,
      title: t('cookiePolicy.sections.cookieManagement.title'),
      content: t('cookiePolicy.sections.cookieManagement.content')
    },
    {
      icon: Shield,
      title: t('cookiePolicy.sections.thirdPartyCookies.title'),
      content: t('cookiePolicy.sections.thirdPartyCookies.content')
    },
    {
      icon: Phone,
      title: t('cookiePolicy.sections.contact.title'),
      content: t('cookiePolicy.sections.contact.content')
    }
  ];
        ...
        <div className="container-custom relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <div className="text-sm text-muted-foreground">
              {t('cookiePolicy.lastUpdated')}: {t('cookiePolicy.updateDate')}
            </div>
          </div>
        </div>
      </section>
            ...
            <Card className="border-2 border-primary/20 bg-primary/5">
              ...
              <CardContent>
                <div className="prose prose-lg max-w-none text-muted-foreground leading-relaxed mb-6">
                  {t('cookiePolicy.settings.content').split('\n').map((paragraph, pIndex) => (
                    <p key={pIndex} className="mb-4 last:mb-0">
                      {paragraph}
                    </p>
                  ))}
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button className="btn-primary rounded-full px-8 py-3">
                    {t('cookiePolicy.settings.acceptAll')}
                  </button>
                  <button className="btn-secondary rounded-full px-8 py-3">
                    {t('cookiePolicy.settings.customize')}
                  </button>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="container-custom">
          <div className="text-center max-w-2xl mx-auto">
            <h2 className="text-3xl font-bold mb-4">
              {t('cookiePolicy.cta.title')}
            </h2>
            <p className="text-muted-foreground mb-8">
              {t('cookiePolicy.cta.subtitle')}
            </p>
